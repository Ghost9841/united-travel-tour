...
export default function AgreementDetailPage() {
  ...
  const [journeyType, setJourneyType] =
    useState<"TWO_WAY" | "ONE_WAY">("TWO_WAY");
  const [termsVersionId, setTermsVersionId] =
    useState<number | "">("");
  ...
  const fetchData = async () => {
    try {
      ...
      if (!agreementResponse.ok) {
        throw new Error(
          agreementData.error || "Failed to load agreement"
        );
      }

      if (!versionsResponse.ok) {
        throw new Error(
          versionsData.error || "Failed to load terms versions"
        );
      }
      ...
      setAirlineName(agreementData.airlineName || "");
      ...
      if (agreementData.departureDate) {
        setDepartureDate(
          new Date(agreementData.departureDate)
            .toISOString()
            .split("T")[0]
        );
      } else {
      ...
      if (agreementData.returnDate) {
        setReturnDate(
          new Date(agreementData.returnDate)
            .toISOString()
            .split("T")[0]
        );
      } else {
  ...
  const saveChanges = async () => {
    try {
      ...
      // Update snapshots when terms version changes
      if (version) {
      ...
      setSectorRoute(data.sectorRoute || "");
      setJourneyType(data.journeyType || "TWO_WAY");
      setTermsVersionId(data.termsVersionId || "");
  ...
  const customerLink =
    typeof window !== "undefined"
      ? `${window.location.origin}/terms-agreement/${id}`
      : "";

  const copyLink = async () => {
    try {
      if (!customerLink) {
        throw new Error("Customer link is not available.");
      }
  ...
  // Determine which terms text to show
  const englishText =
    selectedVersion?.englishText ||
    agreement.englishSnapshot ||
    "No English terms available";

  const nepaliText =
    selectedVersion?.nepaliText ||
      ...
      <div className="mx-auto max-w-5xl space-y-6 p-6">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 p-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-green-700">
                This agreement is locked because the customer
                has already accepted the terms.
              </p>
            </div>
          ...
          <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 p-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-amber-700">
                Send the customer the link to complete the
                agreement.
              </p>
            </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
          ...
          <div className="border-b border-gray-200 px-6 py-5">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {locked
                ? "Read-only - Customer has accepted the terms"
                : "Edit passenger and booking details"}
            </p>
          ...
          <div className="grid gap-5 p-6 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <p className="mt-1 text-xs text-gray-500">
                Changing the version will update the terms
                snapshots.
              </p>
          ...
          {showTerms && (
            <div className="grid gap-6 p-6 md:grid-cols-2">
              ...
              <div>
                ...
                <div className="max-h-96 overflow-y-auto whitespace-pre-wrap rounded-lg bg-gray-50 p-4 text-sm leading-6 text-gray-700">
                  {englishText}
                </div>
